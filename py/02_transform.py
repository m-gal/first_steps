...
# load data
df = pd.read_csv(
    f + "data_in/tf/df_tf_cleaned.csv",
    sep=",",
    decimal=".",
    header=0,
...
cols_x = [
    x for x in list(df) if x not in (cols_inf + cols_y + ["customer_id", "y0_class"])
]
...
for i in range(-1, -n_loans, -1):
    ...
    dshift = (
        df.drop(cols_inf + cols_y, axis=1)
...
dflow = dflow.drop(["customer_id"] + cols_inf, axis=1).join(
    dflow[cols_inf[::-1] + ["customer_id"]]
)
...
#%% Check ----------------------------------------------------------------------
dflow.loc[
    dflow.customer_id.isin(
        [2598448, 2600185, 2599301, 2620070]  # 13 loans  # 20 loans  # 33 loans
    )
].T
...
#%% Save dataset ===============================================================
dflow.to_csv(
    f + "data_in/tf/dflow_" + str(n_loans) + "_loans.csv",
    index=False,
    sep=",",
    decimal=".",
    header=True,
