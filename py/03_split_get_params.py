...
df = pd.read_csv(f + 'data_in/tf/dflow_20_loans.csv', sep=',', decimal='.',
                header=0,
                parse_dates=[
...
print(df.groupby(df['first_status_day_date'].dt.to_period('M')).size().cumsum()\
    /len(df.index))
...
## to embedding cols
cols_x_embe =[x for x in list(df[cols_x]\
    .select_dtypes(include=['object'])) if x not in cols_x_indi]

# Numeric features
## w\o normalizing
binary = ['act_changed_city', 'act_changed_code_zip',
            'act_changed_email_domain', 'act_changed_iban_bank',
            'act_is_name_first_in_email', 'act_is_name_last_in_email',
            'act_is_part_first_name_in_email', 'act_is_part_last_name_in_email',
            'last_sold',
            'last_late_00', 'last_late_03', 'last_late_07',
            'last_late_15', 'last_late_30', 'last_late_60',
            'last_ontime_00', 'last_ontime_03', 'last_ontime_07',
            'last_ontime_15', 'last_ontime_30', 'last_ontime_60',
            'prev_dummy_late_00', 'prev_dummy_late_03', 'prev_dummy_late_07',
            'prev_dummy_late_15', 'prev_dummy_late_30', 'prev_dummy_late_60',
            'prev_dummy_06m_late_00', 'prev_dummy_12m_late_00', 'prev_dummy_24m_late_00'
            ]
cols_x_bina = [x for x in cols_x for y in binary if y in x]
del(binary)
## with normalizing
cols_x_nume = [x for x in list(df[cols_x]\
    .select_dtypes(include=['float64', 'int64'])) if x not in cols_x_bina]

#%% print ----------------------------------------------------------------------
print('\n len cols_x_indi:', len(cols_x_indi), '\n',
    'len cols_x_embe:', len(cols_x_embe), '\n',
    'len cols_x_bina:', len(cols_x_bina), '\n',
    'len cols_x_nume:', len(cols_x_nume), '\n',
    '? Equal for len(cols_x):', len(cols_x), len(cols_x)==len(cols_x_indi\
                                                            +cols_x_embe\
                                                            +cols_x_bina\
                                                            +cols_x_nume)
)
...
df_val = df[(df['first_status_day_date']<data_test) & (df['first_status_day_date']>=data_val)]

#%% print ----------------------------------------------------------------------
len_df = len(df)
print(len(df_test), 'test examples', round(len(df_test)/len_df*100, 2), '% of whole data \n',
       df_test.groupby(['y0_1'])['y0_1'].size().apply(lambda x: x/len(df_test)), '\n')
print(len(df_train), 'train examples', round(len(df_train)/len_df*100, 2), '% of whole data \n',
        df_train.groupby(['y0_1'])['y0_1'].size().apply(lambda x: x/len(df_train)), '\n')
print(len(df_val), 'validation examples', round(len(df_val)/len_df*100, 2), '% of whole data \n',
...
def get_norm_params(df_train, cols):
    ...
    def params(col):
        ...
        q05 = df_train[col].quantile(q=.05)
        q10 = df_train[col].quantile(q=.10)
        q20 = df_train[col].quantile(q=.20)
        q25 = df_train[col].quantile(q=.25)
        q50 = df_train[col].quantile(q=.50)
        q75 = df_train[col].quantile(q=.75)
        q80 = df_train[col].quantile(q=.80)
        q90 = df_train[col].quantile(q=.90)
        q95 = df_train[col].quantile(q=.95)
        return {'min': mini, 'max': maxi,
                'mean': mean, 'std': std,
                'q05': q05, 'q10': q10, 'q20': q20, 'q25': q25, 'q50': q50,
                'q75': q75, 'q80': q80, 'q90':q90, 'q95': q95
        }
...
for col in dic_cols['x']:
    if col in dic_cols['x_nume']:
        ...
        # print(col, 'Indicator')
        gender = feature_column\
            .categorical_column_with_vocabulary_list(col, ['m', 'f'])
        ...
        categorical = feature_column\
            .categorical_column_with_vocabulary_list(col,
                vocabulary_list = vocabulary_list)
        embedding = feature_column\
            .embedding_column(categorical, dimension=dimension)
...
# as data frame
dflow_norm_params.to_csv(f + 'data_in/tf/dflow_norm_params.csv', index=True, sep=','
    , decimal='.', header=True, encoding='utf-8')
...
df_test.to_csv(f + 'data_in/tf/dflow_test.csv', index=False, sep=','
    , decimal='.', header=True)
df_val.to_csv(f + 'data_in/tf/dflow_val.csv', index=False, sep=','
    , decimal='.', header=True)
df_train.to_csv(f + 'data_in/tf/dflow_train.csv', index=False, sep=','
    , decimal='.', header=True)
# ------------------------------------------------------------------------------
print('DONE! Data splitted and params saved for --- %s seconds ---'\
...
print('Min afrer Standard:', df_scaled_std['00_prev_total_profit'].min(), '\n',
        'Max afrer Standard:', df_scaled_std['00_prev_total_profit'].max())
...
print('Min afrer Robust:', df_scaled_rbs['00_prev_total_profit'].min(), '\n',
        'Max afrer Robust:', df_scaled_rbs['00_prev_total_profit'].max())
...
print('Min afrer MinMax:', df_scaled_mmax['00_prev_total_profit'].min(), '\n',
        'Max afrer MinMax:', df_scaled_mmax['00_prev_total_profit'].max())
...
del(MinMaxScaler, RobustScaler, StandardScaler, scaler, col, ax1, ax2, fig,
    df_scaled_std, df_std, df_scaled_rbs, df_robust, df_scaled_mmax, df_minmax)
