...
def plot_loss(history, nn_name):
    ...
    plt.plot(history.epoch,  history.history['val_loss'], label='val '+nn_name,
        linestyle="--")
    # Common parameters
...
#%% Plot: 3 ROC
def plot_3roc(train_target, train_prediction, val_target, val_prediction,
    test_target, test_prediction, nn_name):
    ...
    plt.plot(100*train_fp, 100*train_tp, label='train:auc'+str(train_auc), linewidth=2)
    plt.plot(100*val_fp, 100*val_tp, label='val:auc'+str(val_auc), linewidth=2)
    plt.plot(100*test_fp, 100*test_tp, label='test:auc'+str(test_auc), linewidth=2)
    plt.plot([0, 100], [0, 100],'r--')
...
#%% Plot: Density
def plot_dens(train_target, train_prediction, test_target, test_prediction,
    nn_name):
    fig = plt.figure(figsize=(12,6))
    d = pd.concat([pd.DataFrame({'target':train_target.values,
                            'prediction':train_prediction.ravel(),
                            'data': 'train'}),
                pd.DataFrame({'target':test_target.values,
                            'prediction':test_prediction.ravel(),
                            'data': 'test'})]).groupby(['target', 'data'])

    q50_test_0 = d.quantile(q=.50).values[0]
    q50_test_1 = d.quantile(q=.50).values[2]
    q50_train_0 = d.quantile(q=.50).values[1]
    q50_train_1 = d.quantile(q=.50).values[3]
