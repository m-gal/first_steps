...
print('Train set:\n    \
    Total: {}\n    \
    Positive: {} ({:.2f}% of totall)\n'.format(total, pos, 100 * pos / total))
print('Weight for class 0: {:.2f}'.format(weight_for_0))
print('Weight for class 1: {:.2f}'.format(weight_for_1))
print('\nValidation set:\n  class 0:', np.bincount(df_val['y0_1'])[0],\
    '\n  class 1:', np.bincount(df_val['y0_1'])[1])

#%%[markdown]-------------------------------------------------------------------
#### Start using MLFLOW for for tracking models trainings
#%% Set MLFLOW TRACKING
# SET THE FOLDER - MUST BE named as 'mlruns' --runing only ONCE
EXPERIMENT_PATH = 'file:///T:/Documents/DataProjects/_ferratum/fer_de_ml_ec/mlruns'
...
EXPERIMENT_NAME = 'ffnn'
EXPERIMENT_ID = mlflow.create_experiment(name=EXPERIMENT_NAME)
EXPERIMENT_ID = 3
# ------------------------------------------------------------------------------
print(f'The experiment can be found at {EXPERIMENT_PATH} and has an ID of {EXPERIMENT_ID}')
print('!!! DONT FOGET RUN TERMINAL $ mlflow ui !!!')

#%% RUN EXPERIMENT *************************************************************
SEED=42
NN_NAME = 'ffnn-vanilla'
RUN_NAME=NN_NAME

print(inspect.getsource(sm128m.ffnn_3Dense))
# for CLASS_WEIGHT in [None, CLASS_WEIGHT]:
for OPTIMIZER in ['adam', 'rmsprop', 'sgd']:
    for BATCH_SIZE in [64, 128, 256, 512]:
        for NEURONS in [32, 64, 128, 256, 512, 1024]:
            print(f'\nclass_weight={CLASS_WEIGHT} : optimizer={OPTIMIZER}\
                    \nbatch_size={BATCH_SIZE} : neurons={NEURONS}x{NEURONS}x{NEURONS}')

            tf.set_random_seed(SEED) # for reproducubility
            with mlflow.start_run(experiment_id=EXPERIMENT_ID, run_name=RUN_NAME) as run:

                # BUILD model
                model = sm128m.ffnn_3Dense(feature_cols,
                    neurons=NEURONS,
                    neurons_2=NEURONS,
                    neurons_3=NEURONS,
                ...
                # TRAIN model
                history = sm128m.fit_model(model=model,
                ...
                # EVALUATE model
                train_eval = sm128m.eval_model(model, dataset=ds_train, batch_size=BATCH_SIZE,\
                    dataset_name='train')
                val_eval = sm128m.eval_model(model, dataset=ds_val, batch_size=BATCH_SIZE,\
                    dataset_name='val')
                test_eval = sm128m.eval_model(model, dataset=ds_test, batch_size=BATCH_SIZE,\
                    dataset_name='test')

                # MAKE prediction
                train_predict = sm128m.predict_model(model, dataset=ds_train, batch_size=BATCH_SIZE,\
                    dataset_name='train')
                val_predict = sm128m.predict_model(model, dataset=ds_val, batch_size=BATCH_SIZE,\
                    dataset_name='val')
                test_predict = sm128m.predict_model(model, dataset=ds_test, batch_size=BATCH_SIZE,\
                    dataset_name='test')
                ...
                plot_cm_val = sm128p.plot_cm(target=df_val['y0_1'], prediction=val_predict,
                                            data_name='Val set', nn_name=NN_NAME, p=0.5)
                plot_cm_val.savefig(TEMP_DIR+'plot_cm_val.png', dpi=100)
                # -------- CONFUSION MATRIX for test set
                plot_cm_test = sm128p.plot_cm(target=df_test['y0_1'], prediction=test_predict,
                                            data_name='Test set', nn_name=NN_NAME, p=0.5)
                plot_cm_test.savefig(TEMP_DIR+'plot_cm_test.png', dpi=100)
                # -------- ROC curve
                plot_3roc = sm128p.plot_3roc(train_target=df_train['y0_1'],
                ...
                plot_9x3roc = sm128p.plot_9x3roc(
                    dtrain=df_train[['customer_type', 'act_loan_numinstal', 'y0_1']],
                    train_prediction=train_predict,
                    dval=df_val[['customer_type', 'act_loan_numinstal', 'y0_1']],
                    val_prediction=val_predict,
                    dtest=df_test[['customer_type', 'act_loan_numinstal', 'y0_1']],
                    test_prediction=test_predict,
                    nn_name=NN_NAME)
                plot_9x3roc.savefig(TEMP_DIR+'plot_9x3roc.png', dpi=100)
                # -------- score predicted DENSITY
                plot_dens = sm128p.plot_dens(train_target=df_train['y0_1'],
                                            train_prediction=train_predict,
                                            test_target=df_test['y0_1'],
                                            test_prediction=test_predict,
                                            nn_name=NN_NAME)
                ...
                # --------------------------------------------------------------
                # LOG PARAMETERS
                mlflow.log_param('batchNorm', None)
                mlflow.log_param('batch_size', BATCH_SIZE)
                mlflow.log_param('neurons', str(NEURONS)+'x'+str(NEURONS)+'x'+str(NEURONS))
                ...
                mlflow.log_metric('diff_auc',\
                    history.history['auc'][EPOCHS-1] - history.history['val_auc'][EPOCHS-1])
                mlflow.log_metric('f1',\
                    2*(history.history['precision'][EPOCHS-1]*history.history['recall'][EPOCHS-1])/\
                    (history.history['precision'][EPOCHS-1]+history.history['recall'][EPOCHS-1]))
                ...
                # log MODEL
                model_file = TEMP_DIR+time.strftime("%Y%m%d")+'-'+NN_NAME\
                +'-'+'auc'+str(round(history.history['auc'][EPOCHS-1], 4))\
                +'-'+'val_auc'+str(round(history.history['val_auc'][EPOCHS-1], 4))\
                +'.h5'
